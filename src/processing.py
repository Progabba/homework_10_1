...
def filter_by_state(input_list: list, state: str = "EXECUTED") -> list:
    ...
    result = [i for i in input_list if i.get("state") == state]
    logger.info(f"Результат фильтрации по state: {result}")
    return result
...
def sorted_by_date(input_list: list, ascending: bool = True) -> list:
    ...
    logger.info(f"Результат фильтрации по дате: {sorted_list}")
    return sorted_list
